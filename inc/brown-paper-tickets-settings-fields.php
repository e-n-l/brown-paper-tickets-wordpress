...
class BPTSettingsFields {
	...
	public function get_client_id_input() {
		...
		<div class="bpt-advanced-options">
			...
			<button disabled class="button-secondary button-large">Test Settings</button>
		</div>
		<?php
	}
	...
	public function get_cache_time_input() {
		...
		?>
			<div class="cache-time-wrapper">
				...
				<select id="cache-time-increment" name="_bpt_cache_time[increment]">
					<option value="false" <?php esc_attr_e( selected( $cache_time['increment'], '0' ) );?>>Do Not Cache</option>
					<option value="0" <?php esc_attr_e( selected( $cache_time['increment'], '0' ) );?>>Cache Indefinitely</option>
		<?php 
		while ( $increment < 50 ) {
		...
		?>
				...
				<div class="bpt-advanced-options">
					<p>Click "Refresh Events" to force the plugin to reload your event data.</p>
					<button class="button-large button" id="bpt-refresh-events">Refresh Events</button>
